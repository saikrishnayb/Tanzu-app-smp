...
<fieldset id="user-fieldset" class="user-fieldset">
				<div class="user-form-container">
					<form id="user-form" <c:if test="${isCreatePage == true}"> action="${pageContext.request.contextPath}/admin-console/security/create-user.htm"  </c:if> 
						...
						<input id="ge-sso-id" name="gessouid" type="text" class="displayNone" class="input" value="${editableUser.gessouid}"/>
						<div id="sso-id-div" class="single-line-content">
								<label for="sso-id">SSO ID <span class=requiredField>*</span></label> 
								<input id="sso-id" name="ssoId"  class="input alpha alpha-numeric"  type="text" value="${editableUser.ssoId}"/>
							...
							<label for="bu">Business Unit<span class=requiredField>*</span></label> 
								...
								<c:forEach items="${orgList}" var="org">
									<option value="${org.orgId}"
										...
										<!--  <c:if test="${fn:containsIgnoreCase(org.orgId,currentUser.orgId)}"> selected</c:if>-->
										${org.orgName}</option>
								</c:forEach> 
							</select>
						</div>
