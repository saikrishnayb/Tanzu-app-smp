			  ...
			  success: function(isUnique){
				  ...
				  if(isUnique){
					...
					if(pageAction=="UPDATE" && ruleId!=0){
						url='./update-template-rule.htm';
					}else if(pageAction=="CREATE"){
						url='./create-template-rule.htm';
					}
					$.ajax({
						url : url,
						type: "POST",  
...
function getRuleDetails(ruleId){
	...
	var templateId = $("#template-id").val();
	var $getRulesByComponentId = $.get('get-rule-details.htm', {ruleId:ruleId,templateComponentId:templateComponentId,templateId:templateId});
...
$("#rulesTable tbody").sortable({
	...
	function changeRulePriority(){
		...
		if(ruleList.length>1){
			...
			$.ajax({
				  type: "POST",
				  url: "./update-componet-rules-priority.htm",
				  cache:false,
