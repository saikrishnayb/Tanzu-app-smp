...
//initials file preview modal
	...
	$('.create').on("click", function(){
		...
		if(isNotValid){
			...
			var $createOrgPromise = $.post('./create-org', $orgForm.serialize());
	...
	$('.save').on("click", function(){
		...
		if(isValid){
			...
			//$orgForm.submit();
			var $updateOrgPromise =$.post('./update-org', $orgForm.serialize());
...
function filterVendor(corp,vendor,parentOrg,isReset){
	...
	if (parentOrg.length > 0 && parentOrg >0) {
		...
		var $editUserPromise = $.ajax({
			url: 'filter-vendor-list?_=' + currentTimeStamp,
