 ...
 */
	function validateSession()
	{
		...
		currentTime=(new Date()).getTime();
				...
				var url= getContextRoot()+"/login/validateSession";			//Call the method to make session-alive
				$.ajax({
					  url		: url,
					  type		: 'POST',
					  timeout   :30000,
		    ...
		    error:function(){}
				...
				var smcofurl= "/smcof/validateSession";						//Call the method to make session-alive
				$.ajax({
					  url		: smcofurl,
					  type		: 'POST',
					  timeout   :30000,
		    ...
		    error:function(){}
				...
				var smcopurl= "/smcop/validateSession";						//Call the method to make session-alive
				$.ajax({
					  url		: smcopurl,
					  type		: 'POST',
					  timeout   :30000,
...
function redirect(tabName,templateKey){
	...
	if(path != ""){
		...
		//selectCurrentNavigation(tabName);
		 $.ajax({
			  type: 'POST',
			  url:  getContextRoot()+'/navigation/navigate',
