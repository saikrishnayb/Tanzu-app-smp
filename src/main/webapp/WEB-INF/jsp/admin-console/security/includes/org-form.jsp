...
<fieldset id="user-fieldset" class="user-fieldset-height">
				<div class="org-form-container">
						<form id="org-form"   method="POST" 	<c:if test="${isCreatePage eq true}"> action="${baseAppUrl}/admin-console/security/create-org" </c:if>
						<c:if test="${isCreatePage eq false}"> action="${baseAppUrl}/admin-console/security/update-org" </c:if>> 
				...
				<c:if test="${isCreatePage eq false}">
					<div id="edit-vendor-hierarchy-container" style="height:300px;overflow-y: auto;">
						...
						<div id="edit-vendor-hierarchy" class="floatLeft clear-left jstree">
							<%@ include file="vendor-hierarchy.jsp" %> 
						</div>
					</div>
