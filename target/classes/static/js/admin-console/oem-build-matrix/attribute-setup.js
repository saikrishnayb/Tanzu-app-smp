...
$('#attribute-table').on("click",'.edit-attribute',function(){
		...
		var $getEditAttributeContentPromise =$.get("get-edit-attribute-content",{attributeId:attributeId});
...
$('#attribute-table').on("click",'.add-attribute',function(){
	...
	var $getAddAttributeContentPromise =$.get("get-add-attribute-content",{attributeId:attributeId});
	$('.editAttributeForm[data-save-attr-id="'+attributeId+'"]').remove();
...
$addUpdateAttributeModal.on("click",'#create-attr',function(){
	...
	$.ajax({
		  type: "POST",
		  url: "./check-unique-attribute-value",
		  cache:false,
