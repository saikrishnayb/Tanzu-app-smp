...
function submitLoadSheetForm(){
	...
	if(validateFields()){
			...
			if(seqMasterId=="0"){//Perform Insert
				j2loadSheetSequncingForm.attr("action","create-sequence");
			}else{//perform Update
				if(pageAction == "EDIT"){//Edit Action
					j2loadSheetSequncingForm.attr("action","update-sequence");
...
function reloadUnassignedComponents(category,type){
	processingImageAndTextHandler('visible','Loading data...');
	j2("#loadsheet-sequencing-form").attr("action", "open-create-sequence");  //change the form action
	j2("#loadsheet-sequencing-form").submit();  // submit the form
	
	//window.location.href = './open-create-loadsheet?category='+category+'&type='+type;
