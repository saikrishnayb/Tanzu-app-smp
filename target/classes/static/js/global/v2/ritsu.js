...
var validation = function validation(rules) {
  ...
  var _validateInput = function _validateInput(element) {
    ...
    if (validInput && !isFile && isRequired) validInput = !isEmpty; //Anything is allowed, just can't be blank
  ...
  var _validateSelect = function _validateSelect(element) {
    ...
    if (isAlpha) validSelect = _validateAlphaField(element);
    if (isNumeric) validSelect = _validateNumericField(element);
    if (validSelect && isRequired) validSelect = !isEmpty; //Anything is allowed, just can't be blank
  ...
  var _validateFileField = function _validateFileField(element) {
    ...
    if (isNotRequired) return validFile; //We dont care at all if it is not required (we can't even validate it)
...
var core = function core(rules, validation) {
    ...
    invalidElements.forEach(function(invalidElement) {
      ...
      if (useBootstrapStlying && formGroup != null) {
        ...
        if (defaultOptions.useBootstrap4Stlying) {
          ...
          //If it already has a help text node, add the error message inside it without messing up what is already there
          if (hasHelpBlock) {
  ...
  var _getClosestParentByClass = function _getClosestParentByClass(element, className, subString) {
    ...
    while (element) {
      ...
      var classes = [];
      $.each(parent.classList, function() {
    	  classes.push(this.toString());
      });
      var isClassDesired = classes.some(function(clazz) {
        return subString ? clazz.indexOf(className) > -1 : clazz === className;
      });
  ...
  var _getErrorMessageForInput = function _getErrorMessageForInput(element) {
    ...
    if($(element).is('[required]') && $(element).val() == '')
    	return isDropdown ? 'Please select an option' : 'Please enter a value';
    
    var errorMessage = isDropdown ? 'Please select an option' : 'Invalid Value';

    var elementClassString = element.getAttribute('class');

    var elementHasNoClasses = elementClassString === null || elementClassString === '';
    if (elementHasNoClasses) return errorMessage; //Element has no classes for some reason, so no rule will be found
